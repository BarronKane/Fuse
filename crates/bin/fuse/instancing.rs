...
#[cfg(target_os = "linux")]
use std::os::unix::prelude::RawFd as SYS_INSTANCE;
...
mod instance_temp {
    ...
    impl Instance for InstanceInner {
        ...
        fn create_instance(&mut self, name: &str) -> Result<&Self> {
            let addr = UnixAddr::new_abstract(name.as_bytes()).expect("Couldn't create unix address.");
            let sock = socket::socket(
                socket::AddressFamily::Unix,
                socket::SockType::Stream,
                socket::SockFlag::empty(),
                None,
            ).expect("Couldn't create unix socket.");


            match socket::bind(sock, &addr) {
                Ok(()) => {
                    self.handle = Some(sock);
                    return Ok(self);
                },
                Err(nix::errno::Errno::EADDRINUSE) => {
                    return Err(InstanceError("Handle exists! Is process already running?".to_string()));
                },
                Err(e) => {
                    // TODO: need to catch 'e'. Will refactor errors for this module.
                    let etype = e.desc();
                    let err = format!("Unix socket encountered an error: {}", etype);
                    return Err(InstanceError(err));
                },
            };
        }
    }
